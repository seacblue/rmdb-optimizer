...
Rid RmFileHandle::insert_record(char* buf, Context* context) {
    ...
    if (context != nullptr && context->txn_ != nullptr) {
        context->txn_->append_write_record(new WriteRecord(WType::INSERT_TUPLE, disk_manager_->get_file_name(fd_), rid));
        if (context->log_mgr_ != nullptr) {
            RmRecord insert_record(file_hdr_.record_size, buf);
            InsertLogRecord log(context->txn_->get_transaction_id(), insert_record, rid, disk_manager_->get_file_name(fd_));
            log.prev_lsn_ = context->txn_->get_prev_lsn();
            context->txn_->set_prev_lsn(context->log_mgr_->add_log_to_buffer(&log));
        }
    }
...
void RmFileHandle::delete_record(const Rid& rid, Context* context) {
    ...
    if (context != nullptr && context->txn_ != nullptr) {
        auto record = get_record(rid, nullptr);
        context->txn_->append_write_record(
            new WriteRecord(WType::DELETE_TUPLE, disk_manager_->get_file_name(fd_), rid, *record));
        if (context->log_mgr_ != nullptr) {
            DeleteLogRecord log(context->txn_->get_transaction_id(), *record, rid, disk_manager_->get_file_name(fd_));
            log.prev_lsn_ = context->txn_->get_prev_lsn();
            context->txn_->set_prev_lsn(context->log_mgr_->add_log_to_buffer(&log));
        }
    }
...
void RmFileHandle::update_record(const Rid& rid, char* buf, Context* context) {
    ...
    if (context != nullptr && context->txn_ != nullptr) {
        auto record = get_record(rid, nullptr);
        context->txn_->append_write_record(
            new WriteRecord(WType::UPDATE_TUPLE, disk_manager_->get_file_name(fd_), rid, *record));
        if (context->log_mgr_ != nullptr) {
            RmRecord new_record(file_hdr_.record_size, buf);
            UpdateLogRecord log(
                context->txn_->get_transaction_id(), *record, new_record, rid, disk_manager_->get_file_name(fd_));
            log.prev_lsn_ = context->txn_->get_prev_lsn();
            context->txn_->set_prev_lsn(context->log_mgr_->add_log_to_buffer(&log));
        }
    }
